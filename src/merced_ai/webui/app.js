...
function openBotEditor(bot = null) {
  ...
  openEditor({
    ...
    fields: field("Bot name", "name", bot?.name || "", { required: true, readonly: Boolean(bot) })
      ...
      + field("WebMCP requirement", "requires_webmcp", bot?.harness?.requires_webmcp ? "true" : "false", { select: [{ value: "false", label: "Optional" }, { value: "true", label: "Require native WebMCP" }] }),
    submit: async (values) => api(bot ? `/api/bots/${encodeURIComponent(bot.name)}` : "/api/bots", { method: bot ? "PUT" : "POST", body: JSON.stringify({ ...values, requires_webmcp: values.requires_webmcp === "true", fallbacks: values.fallbacks.split(",").map((item) => item.trim()).filter(Boolean) }) }),
  });
}
